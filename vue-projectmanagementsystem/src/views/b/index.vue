...
export default {        components: { Add, Edit },    data() {        return {                      searchInfo: {                               pageNum: 1,                pageSize: 10,            },            tableData: [],            total: 0,
        ...
        this.gettaskSelectData();
        this.getprojectSelectData();
    },    methods: {
        async getTableData() {
           const res = await this.$axios.post('generate/b/list', this.searchInfo);
            if (res.data.code == 0) {                this.tableData = res.data.data;                this.total = res.data.total;            }        },
        async gettaskSelectData() {
           const res = await this.$axios.post( 'generate/task/allList', {});
            if (res.data.code == 0) {                this.taskSelectData = res.data.data;            }        },
        async getprojectSelectData() {
           ...
           const res = await this.$axios.post('generate/b/del', { id });
