...
@Controller
@CrossOrigin
@RequestMapping(value="/api/generate/enterprise")
public class enterpriseController extends BaseController {
	...
	SysDictService sysDictService;
		...
		public Result save(com.metaShare.modules.generate.entity.enterprise enterprise){
			try {
	            enterprise = this.appendRefInfo(enterprise);
				boolean flag =   enterpriseService.insert(enterprise);
				if(flag){
		...
		public Result update(com.metaShare.modules.generate.entity.enterprise enterprise){
			try {
	            enterprise = this.appendRefInfo(enterprise);
	            boolean flag =   enterpriseService.updateById(enterprise);
	            if(flag){
		...
		public Result del(String id) {
			try {
				boolean flag =   enterpriseService.deleteById(id);
				if(flag){
		...
		public Result getInfo(String id) {
			try {
				com.metaShare.modules.generate.entity.enterprise  enterprise =  enterpriseService.selectById(id);
				return Result.resultInfo(ResultCode.SUCCESS,enterprise);
	       } catch (Exception e) {
	           e.printStackTrace();
		...
		public Result list(int pageNum, int pageSize) {
			try {
				return  enterpriseService.selectList(pageNum,pageSize);
	       } catch (Exception e) {
	           e.printStackTrace();
	    ...
	    public Result allList() {
	        try {
				List<com.metaShare.modules.generate.entity.enterprise> enterpriseList =    enterpriseService.selectList(null);
				return Result.resultInfo(ResultCode.SUCCESS,enterpriseList);
	        } catch (Exception e) {
	            e.printStackTrace();
		...
		private com.metaShare.modules.generate.entity.enterprise appendRefInfo(com.metaShare.modules.generate.entity.enterprise enterprise){
	        try {
	                if(enterprise!=null) {
	                }
	                return enterprise; 
